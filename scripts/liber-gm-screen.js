...
class LiberGMScreen extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static PARTS = {
    navigation: {
      template: "modules/liber-gm-screen/templates/navigation.hbs",
    },
    content: {
      template: "modules/liber-gm-screen/templates/content.hbs",
    },
  };
  ...
  get title() {
    return game.settings.get("liber-gm-screen", "screenTitle") || "Écran du MJ - Liber Chronicles";
  }
  ...
  static async show() {
    const visibleToPlayers = game.settings.get("liber-gm-screen", "visibleToPlayers");
...
Hooks.once("init", () => {
  ...
  game.settings.register("liber-gm-screen", "visibleToPlayers", {
  ...
  game.settings.registerMenu("liber-gm-screen", "menu", {
    name: "Écran du MJ",
...
Hooks.on("renderJournalDirectory", (app, htmlElement) => {
  try {
    ...
    // Évite les doublons
    if (root.querySelector(".liber-gm-btn")) return;

    // Création du bouton
    const btn = document.createElement("button");
    btn.classList.add("liber-gm-btn");
